...
from aioasuswrt.asuswrt import Device as WrtDevice
...
class OpenWrtSensorDataHandler:
    """Data handler for OpenWrt sensor."""

    def __init__(self, hass: HomeAssistant, api: OpenWrt) -> None:
        """Initialize a OpenWrt sensor data handler."""
        self._hass = hass
        self._api = api
...
class OpenWrtDevInfo:
    """Representation of a OpenWrt device info."""

    def __init__(self, mac: str, name: str | None = None) -> None:
        """Initialize a OpenWrt device info."""
        self._mac = mac
        self._name = name
...
class OpenWrtRouter:
    """Representation of a OpenWrt router."""

    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        """Initialize a OpenWrt router."""
        self.hass = hass
        self._entry = entry

        self._api: OpenWrt = None
        self._protocol: str = entry.data[CONF_PROTOCOL]
        self._host: str = entry.data[CONF_HOST]
        self._model: str = "OpenWrt Router"
        self._sw_v: str | None = None

        self._devices: dict[str, OpenWrtDevInfo] = {}
        self._connected_devices: int = 0
        self._connect_error: bool = False

        self._sensors_data_handler: OpenWrtSensorDataHandler | None = None
    ...
    async def setup(self) -> None:
        ...
        for entry in track_entries:
            ...
            self._devices[device_mac] = OpenWrtDevInfo(device_mac, entry.original_name)
    ...
    async def update_all(self, now: datetime | None = None) -> None:
        """Update all OpenWrt platforms."""
        await self.update_devices()

    async def update_devices(self) -> None:
        """Update OpenWrt devices tracker."""
        new_device = False
        _LOGGER.debug("Checking devices for OpenWrt router %s", self._host)
        try:
            ...
            if not self._connect_error:
                ...
                _LOGGER.error(
                    "Error connecting to OpenWrt router %s for device update: %s",
                    self._host,
                    exc,
                )
            return
        ...
        if self._connect_error:
            ...
            _LOGGER.info("Reconnected to OpenWrt router %s", self._host)
        ...
        for device_mac, dev_info in wrt_devices.items():
            if not track_unknown and not dev_info.name:
                continue
            new_device = True
            device = OpenWrtDevInfo(device_mac)
    ...
    async def init_sensors_coordinator(self) -> None:
        """Init OpenWrt sensors coordinators."""
        if self._sensors_data_handler:
            return

        self._sensors_data_handler = OpenWrtSensorDataHandler(self.hass, self._api)
    ...
    async def _update_unpolled_sensors(self) -> None:
        """Request refresh for OpenWrt unpolled sensors."""
    ...
    async def _get_available_temperature_sensors(self) -> list[str]:
        ...
        try:
            ...
            _LOGGER.debug(
                (
                    "Failed checking temperature sensor availability for OpenWrt router"
                    " %s. Exception: %s"
                ),
                self._host,
                exc,
    ...
    def device_info(self) -> DeviceInfo:
        ...
        return DeviceInfo(
            identifiers={(DOMAIN, self.unique_id or "OpenWrt")},
            name=self._host,
            model=self._model,
    ...
    def signal_device_new(self) -> str:
        """Event specific per OpenWrt entry to signal new device."""
        return f"{DOMAIN}-device-new"

    @property
    def signal_device_update(self) -> str:
        """Event specific per OpenWrt entry to signal updates in devices."""
    ...
    @property
    def devices(self) -> dict[str, OpenWrtDevInfo]:
        """Return devices."""
...
async def get_nvram_info(api: OpenWrt, info_type: str) -> dict[str, Any]:
    """Get OpenWrt router info from nvram."""
    info = {}
    try:
...
def get_api(conf: dict[str, Any], options: dict[str, Any] | None = None) -> OpenWrt:
    """Get the OpenWrt API."""
    opt = options or {}
