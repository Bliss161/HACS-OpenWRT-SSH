...
from .router import OpenWrtDevInfo, OpenWrtRouter
...
async def async_setup_entry(
    ...
    """Set up device tracker for OpenWrt component."""
...
@callback
def add_entities(
    router: OpenWrtRouter, async_add_entities: AddEntitiesCallback, tracked: set[str]
) -> None:
    ...
    for mac, device in router.devices.items():
        ...
        new_tracked.append(OpenWrtDevice(router, device))
        tracked.add(mac)
...
class OpenWrtDevice(ScannerEntity):
    """Representation of a OpenWrt device."""

    _attr_should_poll = False

    def __init__(self, router: OpenWrtRouter, device: OpenWrtDevInfo) -> None:
        """Initialize a OpenWrt device."""
        self._router = router
        self._device = device
